...
const getFavorites = async (token) => {
    await webSync(token).then((r) => { console.log('Synced with Manga Rock'); });
    return axios_1.default({
        method: 'POST',
...
exports.verifyPassword = (email, password) => {
    return axios_1.default({
        method: 'POST',
...
exports.signIn = async (email, password, proxy = false) => {
    let options = {};
    if (proxy) {
        options = {
            method: 'POST',
    ...
    return axios_1.default(options).then((res) => {
        return res.data.token;
    });
};
exports.getMangaInfo = async (token) => {
